...
app.get("/getlast10transaction/:idUser", function (req, res) {
    const idUser = req.params.idUser
    const query = "SELECT INCOME.idUser, CATEGORY.name_category, INCOME.name_product, INCOME.price, INCOME.created_at FROM INCOME,CATEGORY WHERE CATEGORY.categoryId = INCOME.categoryId UNION SELECT OUTCOME.idUser,CATEGORY.name_category, OUTCOME.name_product, OUTCOME.price, OUTCOME.created_at FROM OUTCOME,CATEGORY WHERE CATEGORY.categoryId = OUTCOME.categoryId HAVING idUser= ? ORDER BY created_at DESC LIMIT 10"
    connection.query(query, [idUser], function (err, rows) {
        if (err) {
            res.status(500).send({ message: err.sqlMessage })
        } else {
...
app.get("/gettrasaction/:idUser", function (req, res) {
    const idUser = req.params.idUser
    const query = "SELECT INCOME.idUser, CATEGORY.name_category, INCOME.name_product, INCOME.price, INCOME.created_at FROM INCOME,CATEGORY WHERE CATEGORY.categoryId = INCOME.categoryId UNION SELECT OUTCOME.idUser,CATEGORY.name_category, OUTCOME.name_product, OUTCOME.price, OUTCOME.created_at FROM OUTCOME,CATEGORY WHERE CATEGORY.categoryId = OUTCOME.categoryId HAVING idUser= ? ORDER BY created_at DESC "
    connection.query(query, [idUser], function (err, rows) {
        if (err) {
            res.status(500).send({ message: err.sqlMessage })
        } else {
